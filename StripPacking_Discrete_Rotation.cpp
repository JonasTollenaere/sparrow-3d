...
public:
    ...
    std::shared_ptr<EnhancedStripPackingSolution> explore(std::shared_ptr<EnhancedStripPackingSolution>& solution, float initialHeight) {
        ...
        while (elapsedMilliseconds < 60 * 60 * 1000 && !stopCalled) {
            ...
            elapsedMilliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - start;
        }
