...
float computeSignedDistanceToMeshAndPoles(const std::shared_ptr<BoundingVolumeHierarchy>& tree, const glm::vec3& point, const std::vector<Sphere>& poles, double minimumDistanceRequired){
...
Sphere InaccessibilityPoles::findNextPoleOfInaccessibility(const std::shared_ptr<ModelSpaceMesh> &mesh, const std::vector<Sphere> &previousPoles, const std::shared_ptr<BoundingVolumeHierarchy> &tree) {
    ...
    auto firstDistance = computeSignedDistanceToMeshAndPoles(tree, firstPoint, previousPoles, -std::numeric_limits<float>::max());
