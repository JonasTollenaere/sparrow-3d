...
void run(RenderWidget* renderWidget) {
    ...
    instances.emplace_back(STOYAN_2005_EXAMPLE_3, 28.83);
    ...
    instances.emplace_back(LAMAS_FERNANDEZ_2022_ENGINE, 0.0);
    ...
    for (int seed = 0; seed < 15; ++seed) {
        for (const auto& [instance, bestReportedHeight] : instances) {
            ...
            FixedRotationStripPackingTask task(StripPackingProblem::fromInstancePath(instance, ObjectOrigin::AlignToMinimum), options);
            ...
            std::ofstream jsonOutputFile(std::string(SOLUTION_DIR) + "fixed/" + result->getProblem()->getName() + "_Fixed_" + std::to_string(seed) + "_" + std::to_string(result->computeTotalHeight()) + ".json");
            jsonOutputFile << solutionJSON.dump(4);
            jsonOutputFile.close();

            // Check if CSV file exists before opening
            std::string csvFilePath = std::string(SOLUTION_DIR) + "fixed/benchmark_results_Fixed.csv";
