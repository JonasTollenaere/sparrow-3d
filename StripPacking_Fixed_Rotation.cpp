...
public:
    explicit Task(const std::shared_ptr<StripPackingProblem>& problem): problem(problem) {}

    bool separate(std::shared_ptr<EnhancedStripPackingSolution>& solution, size_t maxAttempts, size_t maxIterationsWithoutImprovement, float currentHeight, Random& random) const {
        ...
        while (n < maxAttempts && !stopCalled) {
    ...
    std::shared_ptr<EnhancedStripPackingSolution> explore(std::shared_ptr<EnhancedStripPackingSolution>& solution, float initialHeight) const {
        ...
        while (elapsedTime < 60 * 60 * 1000 && !stopCalled) {
            ...
            elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - start;
        }
